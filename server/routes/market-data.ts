...
router.get('/search', async (req, res) => {
  try {
    ...
    if (queryParams.metro_area) {
      whereClause.metro_area = { contains: queryParams.metro_area };
    }
    
    if (queryParams.region) {
      whereClause.region = { contains: queryParams.region };
    }
...
router.get('/cap-rates/:property_type', async (req, res) => {
  try {
    ...
    if (region) {
      whereClause.region = { contains: region as string };
    }

    if (metro_area) {
      whereClause.metro_area = { contains: metro_area as string };
    }
...
async function getLatestMarketData(filter: { zip_code?: string; metro_area?: string }) {
  ...
  if (filter.metro_area) {
    whereClause.metro_area = { contains: filter.metro_area };
  }
