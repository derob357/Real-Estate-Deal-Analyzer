      ...
      propertyElements.forEach((element) => {
        ...
        if (priceElement) {
          ...
          if (priceText) {
            ...
            if (priceMatch) {
              property.listingPrice = Number.parseFloat(priceMatch[1].replace(/,/g, ''));
            }
          }
        }
        ...
        if (capRateElement) {
          ...
          if (capRateText) {
            ...
            if (capRateMatch) {
              property.capRate = Number.parseFloat(capRateMatch[1]) / 100;
            }
          }
        }
      ...
      transactionElements.forEach((element) => {
        ...
        if (salePriceElement) {
          ...
          if (priceText) {
            const priceMatch = priceText.match(/\$?([\d,]+(?:\.\d{2})?)/);
            if (priceMatch) {
              transaction.salePrice = Number.parseFloat(priceMatch[1].replace(/,/g, ''));
            }
          }
        }
        ...
        if (capRateElement) {
          ...
          if (capRateText) {
            const capRateMatch = capRateText.match(/(\d+\.?\d*)%?/);
            if (capRateMatch) {
              transaction.capRate = Number.parseFloat(capRateMatch[1]) / 100;
            }
          }
        }
    ...
    const marketData = await page.evaluate(() => {
      ...
      capRateElements.forEach((element) => {
        ...
        if (propertyType && capRate) {
          ...
          if (rateMatch) {
            data.capRates[propertyType.toLowerCase()] = Number.parseFloat(rateMatch[1]) / 100;
          }
        }
      });
