...
export async function GET(
  ...
  try {
    ...
    const years = yearsParam ? Number.parseInt(yearsParam) : 5
    ...
    if (Number.isNaN(years) || years < 1 || years > 20) {
      return NextResponse.json(
        { error: 'Years must be a number between 1 and 20' },
        { status: 400 }
      )
    }
    ...
    const taxTrends = calculateTaxTrends(assessments, payments)

    // Organize data by year
    const yearlyData = organizeByYear(assessments, payments)
    ...
    return NextResponse.json({
      ...
      tax_history: {
        ...
        summary: {
          ...
          average_annual_tax: calculateAverageAnnualTax(assessments),
