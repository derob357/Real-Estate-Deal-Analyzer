...
import { Search, Building2, Calculator, TrendingUp, MapPin, DollarSign, Bell, BarChart3, Menu, X, User, Settings } from "lucide-react"
...
import { EnhancedMLSService } from "@/services/EnhancedMLSService"
...
type ActiveTab = 'search' | 'alerts' | 'portfolio' | 'manage'
...
export default function MobileResponsiveDashboard() {
  ...
  const [enhancedMLSService] = useState(() => EnhancedMLSService.getInstance())
  const [realEstateAPIService] = useState(() => RealEstateAPIService.getInstance())
  ...
  useEffect(() => {
    ...
    const initializeServices = async () => {
      try {
        await Promise.all([
          ...
          enhancedMLSService.initialize(),
          realEstateAPIService.initialize()
        ])
        console.log('🎯 All real estate services initialized')
      } catch (error) {
    ...
    initializeServices()
  }, [mlsService, enhancedMLSService, realEstateAPIService])
  ...
  const handleZipCodeSearch = async (zipCode: string) => {
    ...
    try {
      console.log('🔍 Searching properties with enhanced MLS and API services...')

      // Use enhanced MLS service for comprehensive search
      const enhancedResults = await enhancedMLSService.searchCommercialProperties({
        location: {
          zipCodes: [zipCode]
        },
        property: {},
      ...
      console.log(`📊 Enhanced MLS found ${enhancedResults.totalCount} properties from ${enhancedResults.sources.length} sources`)

      if (enhancedResults.properties.length > 0) {
        const convertedProperties: Property[] = enhancedResults.properties.map(prop => ({
          id: prop.id,
          ...
          noi: prop.noi,
          listingSource: prop.source,
          mlsId: prop.mlsId,
          daysOnMarket: prop.daysOnMarket,
          photos: prop.photos,
          distance: 0 // Will be calculated for radius searches
        }))
        setProperties(convertedProperties)
      } else {
        // Try fallback with original MLS service
        console.log('🔄 Trying fallback MLS service...')
  ...
  const NavigationMenu = () => (
    <nav className="flex flex-col space-y-2 p-4">
      ...
      <button
        ...
        className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors text-left ${
          activeTab === 'manage' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'
        }`}
      >
        <Settings className="h-5 w-5" />
        <div>
          <div className="font-medium">Manage Properties</div>
          <div className="text-sm opacity-75">Add & edit listings</div>
        </div>
      </button>
    </nav>
  )
      ...
      <CardContent className="p-4">
        <div className="space-y-3">
          ...
          <div className="aspect-video bg-gray-200 rounded-lg overflow-hidden">
            <img
              src={getPropertyImageUrl({ images: property.photos }, property.id)}
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="hidden md:block border-t">
          <div className="flex items-center justify-center space-x-1 py-2">
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors ${
                activeTab === 'manage' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 md:py-8">
        <SignedIn>
          ...
          {activeTab === 'manage' && <PropertyManagement />}
        </SignedIn>
