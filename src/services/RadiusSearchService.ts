...
export class RadiusSearchService {
  ...
  private static generateMockZipCodesInRadius(center: ZipCodeLocation, radiusMiles: number): ZipCodeLocation[] {
    ...
    for (let i = 0; i < numZipCodes; i++) {
      ...
      const baseZip = Number.parseInt(center.zipCode)
   ...
   */
  static generateMockProperties(zipCode: string, count = 3): Property[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      properties.push({
        ...
        photos: [apartmentComplexImages[(i + Math.floor(Math.random() * apartmentComplexImages.length)) % apartmentComplexImages.length]]
      })
    }
